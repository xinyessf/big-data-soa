...
object CmccFenye {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // val results: DataFrame = spark.sql(sql1)
     ...
     //val result4: DataFrame = spark.sql("select count(1) from original.t_ods_cmcc_block_sm u where day='20200521'")
   ...
   // val result4: Dataset[Row] = spark.sql("select * from original.t_ods_cmcc_block_sm where day='20200521'")
   // val result4: DataFrame = spark.sql("select row_number() over (order by c_time desc) as rownum,u.* from original.t_ods_cmcc_block_sm u where day='20200520'")
    ...
    val rows: Array[Row] = rdd.take(50000)
    for (row<-rows){
      println(row.get(0))
    }
    //rows.collect();
