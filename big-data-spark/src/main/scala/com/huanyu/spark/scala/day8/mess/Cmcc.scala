...
object Cmcc {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //删表
   // val results22: DataFrame = spark.sql("create database  iaf")

    //val results20: DataFrame = spark.sql("drop table iaf.t_ods_cmcc_block_sm")
    // val sql1 = "CREATE TABLE iaf.t_ods_cmcc_block_sm(c_usernum STRING COMMENT '主叫号码',c_relatenum STRING COMMENT '被叫号码',c_time STRING COMMENT '时间',c_content STRING COMMENT '短信内容') PARTITIONED BY (day string) row format delimited fields terminated by ','";
    ...
    //val results: DataFrame = spark.sql(sql1)
     ...
     //val sql: DataFrame = spark.sql("load data local inpath 'E:/wordcount/cmcc/5.txt'  into table iaf.t_ods_cmcc_block_sm partition(day='20200525')")
    ...
    val result4: Dataset[Row] = spark.sql("select count(1) from iaf.t_ods_cmcc_block_sm where day='20200525'")
